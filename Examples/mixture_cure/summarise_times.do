clear
cd "/media/paul/Storage/GitSoftware/mlad/Examples/mixture_cure"
...
foreach ss in $sampsizes {
  if `i'==1 {
    use Results/mixture_cure_results_`ss'
  } 
  else {
    append using Results/mixture_cure_results_`ss'
  }
  local i = `i' + 1
}
...
twoway (line mtime sampsize if method=="jaxjit_pro1") ///
       (connected mtime sampsize if method=="strsmix_pro1") ///
       , 

preserve
  keep if inlist(method,"jaxjit_pro1","strsmix_pro1")
