...
def tanhsinh_quad(Nnodes,N):
  ...
  h = delta[1] - delta[0]
  nodes = jnp.tanh(jnp.pi*0.5*jnp.sinh(delta))
  weights = (0.5*h*jnp.pi*jnp.cosh(delta)) / ((jnp.cosh(0.5*jnp.pi*jnp.sinh(delta))**2))
...
# B-splines
def bs(x,degree,knots,bknots,calcintercept,calcderiv,addconstant):
  Nobs = x.shape[0]
  
  fullknots = jnp.concatenate((jnp.repeat(bknots[0],degree+1),
                               jnp.asarray(knots)*jnp.ones(1),
                               jnp.repeat(bknots[1],degree+1)))
  Nintervals = fullknots.shape[0] - 1
  M1 = jnp.zeros((Nobs,Nintervals)) 
  for i in range(Nintervals):
    M1 = M1.at[:,i].set(((fullknots[i]<=x) * (x<fullknots[i+1])) + 
                         (i>=(Nintervals-degree-1))*(fullknots[i+1]==x))
  for pindex in range(degree):
    p = pindex + 1
    splinevars = jnp.zeros((Nobs,Nintervals - p)) 
    for i in range(Nintervals - p):
      C1 = jnp.where(fullknots[i+p] == fullknots[i],0,
                    (x-fullknots[i])/(fullknots[i+p]-fullknots[i]))
      C2 = jnp.where(fullknots[i+p+1]==fullknots[i+1],0,
                    (fullknots[i+p+1]-x)/(fullknots[i+p+1]-fullknots[i+1]))
      splinevars = splinevars.at[:,i].set(C1 * M1[:,i] + C2 * M1[:,i+1])
    if p!=degree:
      M1 = splinevars
  ...
  if calcderiv:
    dsplinevars = jnp.zeros((Nobs,Nintervals-degree))
    for i in range(Nintervals - degree):
      C1 = jnp.where(fullknots[i+degree]==fullknots[i],0,
                     degree/(fullknots[i+degree]-fullknots[i]))
      C2 = jnp.where(fullknots[i+degree+1]==fullknots[i+1],0,
                     degree/(fullknots[i+degree+1]-fullknots[i+1]))
      dsplinevars = dsplinevars.at[:,i].set(C1*M1[:,i] - C2*M1[:,i+1])

    if not calcintercept:
      dsplinevars = dsplinevars[:,1:] 

  if addconstant:
    splinevars = jnp.hstack((bsplinevars,jnp.ones((Nobs,1))))
    if calcderiv: jnp.hstack((dsplinevars,jnp.zeros((Nobs,1))))      
...
## Natural splines
def ns(x,knots,bknots,calcintercept,calcderiv,addconstant):
  Nobs = x.shape[0]
  Nintk = len(knots)
  Nk = Nintk + 2
  ...
  if calcderiv:
    bsplines, dbsplines = bs(x,3,knots,bknots,1,1,0) 
  else:
    bsplines = bs(x,3,knots,bknots,1,0,0)
  
  # deal with boundaries 
  bs_bknots, dbs_bknots = bs(jnp.array(bknots),3,knots,bknots,1,1,0)

  bsplines = jnp.where(x[:,None]<bknots[0],
                       bs_bknots[0,:] + dbs_bknots[0,:][None]*(x[:,None] - bknots[0]),
                      bsplines)  
  bsplines = jnp.where(x[:,None]>bknots[1],
                       bs_bknots[1,:] + dbs_bknots[1,:][None]*(x[:,None] - bknots[1]),
                      bsplines)
  if calcderiv:
    dbsplines = jnp.where(x[:,None]<bknots[0],
                          dbs_bknots[0,:],
                          dbsplines)
    dbsplines = jnp.where(x[:,None]>bknots[1],
                          dbs_bknots[1,:],
                          dbsplines)  
  ...
  if calcderiv: dbsplines = jnp.matmul(dbsplines,H)
  if not calcintercept: 
    bsplines = bsplines[:,:-1]
    if calcderiv: dbsplines = dbsplines[:,:-1]
    
  if addconstant:
    bsplines = jnp.hstack((bsplines,jnp.ones((Nobs,1))))
    if calcderiv: jnp.hstack((dbsplines,jnp.zeros((Nobs,1))))
...
vbs = jit(jax.vmap(bs,(0,None,None,None,None,None,None)) ,static_argnums=(1,4,5,6))
vns = jit(jax.vmap(ns,(0,None,None,None,None,None)),static_argnums=(3,4,5))

# bs2 for comparison
def bs2(x,degree,knots,bknots,calcintercept,calcderiv):
  fullknots = jnp.concatenate((jnp.repeat(bknots[0],degree+1),
                               jnp.asarray(knots)*jnp.ones(1),
...
def ns2(x,knots,bknots,calcintercept,calcderiv):
  Nintk = len(knots)
  Nk = Nintk + 2
  allknots = jnp.hstack((jnp.repeat(bknots[0],4)[None,:],
                              jnp.array(knots)[None,:],
  ...
  if calcderiv:
    bsplines, dbsplines = bs2(x,3,knots,bknots,1,1) 
  else:
    bsplines = bs2(x,3,knots,bknots,1,0)

  # deal with boundaries 
  bs_bknots, dbs_bknots = bs2(jnp.array(bknots),3,knots,bknots,1,1)
  Nobs = x.shape[0]
  x = x[:,None] 
...
# vectorized bsplines and natural splines
#### ADD BACK JIT ####
vbs2 = jit(jax.vmap(bs2,(0,None,None,None,None,None)) ,static_argnums=(1,4,5))
vns2 = jit(jax.vmap(ns2,(0,None,None,None,None)), static_argnums=(3,4))
